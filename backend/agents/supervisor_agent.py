...
class Supervisor:
    ...
    def _get_prompts(self, list_agents, agent_descriptions_text):
        system_prompt = (
    ...
    def perform_task(self, state):
        try:
            ...
            agents_available = list(self.agent_descriptions.keys())
            ...
            agent_descriptions_text = "\n".join(
                [f"{agent}: {description}" for agent, description in self.agent_descriptions.items()]
            )

            prompt = ChatPromptTemplate.from_messages([
                ("system", self._get_prompts(
            ...
            supervisor = current_prompt | self.model.with_structured_output(SupervisorOutput)
